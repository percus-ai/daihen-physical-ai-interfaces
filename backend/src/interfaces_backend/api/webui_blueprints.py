...
_BLUEPRINT_SUMMARY_COLUMNS = "id,name,created_at,updated_at"
_BLUEPRINT_DETAIL_COLUMNS = f"{_BLUEPRINT_SUMMARY_COLUMNS},blueprint"
_BLUEPRINT_ID_COLUMNS = "id"
_BINDING_SUMMARY_COLUMNS = "blueprint_id,last_used_at"
_BINDING_SESSION_COLUMNS = "blueprint_id"
_BINDING_ID_COLUMNS = "id"
...
def _as_blueprint_summary(row: dict[str, Any]) -> WebuiBlueprintSummary:
    ...
    if not isinstance(name, str) or not name.strip():
        raise HTTPException(
            status_code=500, detail="Invalid blueprint row: missing name"
        )
...
def _as_blueprint_detail(row: dict[str, Any]) -> WebuiBlueprintDetail:
    ...
    if not isinstance(blueprint, dict):
        raise HTTPException(
            status_code=500, detail=f"Blueprint payload is invalid: {summary.id}"
        )
    try:
...
async def _list_blueprint_rows(client: Any, user_id: str) -> list[dict[str, Any]]:
    response = (
        ...
        .select(_BLUEPRINT_SUMMARY_COLUMNS)
...
async def _list_binding_rows(client: Any, user_id: str) -> list[dict[str, Any]]:
    response = (
        ...
        .select(_BINDING_SUMMARY_COLUMNS)
        .eq("owner_user_id", user_id)
...
async def _get_optional_blueprint_row(
    client: Any, user_id: str, blueprint_id: str
) -> dict[str, Any] | None:
    rows = (
        await client.table(_BLUEPRINTS_TABLE)
        .select(_BLUEPRINT_DETAIL_COLUMNS)
        .eq("owner_user_id", user_id)
        .eq("id", blueprint_id)
        .limit(1)
        .execute()
...
async def _get_blueprint_row(
    client: Any, user_id: str, blueprint_id: str
) -> dict[str, Any]:
    row = await _get_optional_blueprint_row(client, user_id, blueprint_id)
    if row is None:
        raise HTTPException(
...
async def _ensure_blueprint_exists(
    client: Any, user_id: str, blueprint_id: str
) -> None:
    rows = (
        await client.table(_BLUEPRINTS_TABLE)
        .select(_BLUEPRINT_ID_COLUMNS)
        .eq("owner_user_id", user_id)
        .eq("id", blueprint_id)
        .limit(1)
        .execute()
...
async def _get_binding_row(
    ...
    rows = (
        await client.table(_BINDINGS_TABLE)
        .select(_BINDING_SESSION_COLUMNS)
        .eq("owner_user_id", user_id)
...
async def _resolve_blueprint(
    ...
    *,
    ...
    if binding:
        ...
        bound_row = await _get_optional_blueprint_row(client, user_id, bound_id)
    ...
    if last_used_id and last_used_id in blueprint_by_id:
        last_used_row = await _get_optional_blueprint_row(client, user_id, last_used_id)
        if last_used_row is not None:
            await _upsert_binding(
                client,
                user_id,
                session_kind=session_kind,
            ...
            return last_used_row, "last_used"

    latest_id = _select_latest_blueprint_id(blueprints)
    if latest_id and latest_id in blueprint_by_id:
        latest_row = await _get_optional_blueprint_row(client, user_id, latest_id)
...
async def list_blueprints():
    ...
    last_used_blueprint_id = _derive_last_used_blueprint_id(
        binding_rows, blueprint_by_id
    )
...
async def update_blueprint(blueprint_id: str, request: WebuiBlueprintUpdateRequest):
    ...
    await _ensure_blueprint_exists(client, user_id, blueprint_id)
