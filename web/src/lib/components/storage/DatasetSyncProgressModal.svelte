<script lang="ts">
  ...
  import { api, type DatasetSyncJobStatus } from '$lib/api/client';
  ...
  let status = $state<DatasetSyncJobStatus | null>(null);
  ...
  $effect(() => {
    ...
    contributor = registerTabRealtimeContributor({
      subscriptions: [
        {
          subscription_id: `storage.dataset-sync.modal.${jobId}`,
...
<AlertDialog.Root bind:open={open}>
  <AlertDialog.Portal>
    ...
    >
      <AlertDialog.Title class="text-base font-semibold text-slate-900">データセット同期進捗</AlertDialog.Title>
      <AlertDialog.Description class="mt-2 text-sm text-slate-600">
        データセットのローカル同期状況を表示しています。
